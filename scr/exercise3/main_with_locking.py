...
def run_schedule_S1(isolation_level: int = ISOLATION_LEVEL_READ_COMMITTED):
    ...
    # r1(x)
    cur1.execute("SELECT * FROM sheet3 WHERE id = 1 FOR SHARE;")
    cur1.execute("SELECT name FROM sheet3 WHERE id = 1;")
    print(f"r1(x): {cur1.fetchone()[0]}")

    # w2(x)
    cur2.execute("SELECT * FROM sheet3 WHERE id = 1 FOR UPDATE;")
...
def run_schedule_S3(isolation_level: int = ISOLATION_LEVEL_READ_COMMITTED):
    ...
    # r2(x)
    cur2.execute("SELECT * FROM sheet3 WHERE id = 1 FOR SHARE;")
    ...
    # w1(y)
    cur1.execute("SELECT * FROM sheet3 WHERE id = 2 FOR UPDATE;")
    ...
    # r2(y)
    cur2.execute("SELECT * FROM sheet3 WHERE id = 2 FOR SHARE;")
    ...
    # w2(y)
    cur2.execute("SELECT * FROM sheet3 WHERE id = 2 FOR UPDATE;")
